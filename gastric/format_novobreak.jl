        ...
        if startswith(line, "#")
            ...
            if CHROM ∉ keys(shunxu) || CHROM2 ∉ keys(shunxu)
                ...
                elseif svclass == "DEL"
                    ...
                    println(IO, CHROM, "\t", POS, "\t", POS + 1, "\t", CHROM2, "\t", POS2, "\t", POS2 + 1, "\t", ID, "\t", pesupport, "\t", "-", "\t", "+", "\t", "INS/DUP", "\t", "NovoBreak")
                end
            end
        end
    end
...
function main()
    for fs in Glob.glob("NovoBreak/*_novobreak.vcf")
        prx = replace(fs, "_novobreak.vcf" => "", "NovoBreak/" => "")
        open("format_novo/format.$(prx).sv.novobreak.txt", "w+") do io
