...
getNums<-function(sample){
  df<-read_tsv(stringr::str_glue("FinallyData/plasma/filterData/{sample}_circle_site.filter.tsv"))
  nrow(df)
}
...
fin<-read_tsv("FinallyData/plasma/eccnumbers.tsv")
mappings<-read_tsv("FinallyData/plasma/eccmaps.tsv")
...
ggsave("plasma.mapping.png",width =5.9 ,height = 4.54)

fin<-fin%>%
  mutate(EPM=eccnumbers/totalmappings*10^6)
openxlsx::write.xlsx(fin,"FinallyData/tissue/Tissue_circle_epms.xlsx")
...
## save length 
save(fin,file="FinallyData/plasma/allplasma_length_total.Rdata")
...
ggsave("FinallyData/plasma/length.plot.pdf",width =6.56 ,height =3.42)  
...
## circle plot
## 7.13e-9, 9.69e-3, 7.77e-10, 1.18e-7
...
ggsave("FinallyData/plasma/length_cat.plot.pdf",width =7.49 ,height =3.43) 

##GC content
readGC<-function(x){
  df<-read_tsv(stringr::str_glue("FinallyData/plasma/GCs/{x}.gcContents.txt"))%>%
...
saveRDS(plotData2,"FinallyData/plasma/plasma.gc.Rds")
...
ggsave("FinallyData/plasma/GC_ridges.pdf",width = 6.09,height = 4.16)

# ecc per chrom -----------------------------------------------------------

stat_chromo_num<-function(x) {
  data<-read_tsv(stringr::str_glue("FinallyData/plasma/filterData/{x}_circle_site.filter.tsv"))
...
chromosome_size<-read_tsv("FinallyData/plasma/dbs/hg38.chromo.size",col_names = F)
...
write_tsv(stat,"FinallyData/plasma/ecc_chromosome_pvalue.tsv")
ggsave("FinallyData/plasma/ecc_chromosome.pdf",width = 8.61,height = 3.23)

##oncogenes
##ecc abundance
genes<-read_tsv("FinallyData/plasma/dbs/hg38.coding.bed",col_names = F)
names(genes)<-c("chr","Start","End","gene")
genes<-genes%>%
  mutate(length=End-Start)
get_genes<-function(x,dbs=genes){
  df<-read_tsv(stringr::str_glue("FinallyData/plasma/start_anno/{x}.startAnno.bed"),col_names = F)
  df<-df%>%
    dplyr::select(1:4,8)%>%
...
saveRDS(fin,"FinallyData/plasma/gene_drops.RDS")
...
saveRDS(dffs,"FinallyData/plasma/gene_drops.diff.RDS")
...
pdf("FinallyData/plasma/gene_drops.pdf",width = 5.69,height = 5.44)
draw(ht)
dev.off()
...
dffs<-readRDS("FinallyData/plasma/gene_drops.diff.RDS")
fin<-readRDS("FinallyData/plasma/gene_drops.RDS")
topGenes<-nima%>%
  filter(logfc<0)%>%
  pull(gene)
#lowGenes<-nima%>%
...
openxlsx::write.xlsx(result,"FinallyData/plasma/diff_genes.enrich.xlsx")
...
find_intact<-function(x){
  ## readin ecc
  df<-read_tsv(stringr::str_glue("FinallyData/plasma/filter_bed/{x}.ecc.bed"),col_names = F)
...
saveRDS(haha,"FinallyData/plasma/miRNA_statMat.RDS")
write_tsv(haha,"FinallyData/plasma/miRNA_statMat.tsv")
...
pdf("FinallyData/plasma/ecc-miRNA.pdf",width = 8/09,height = 4.17)
draw(ht)
dev.off()
